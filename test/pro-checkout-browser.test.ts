...
test("native Pro checkout isolates its Host session from public content, stages a mainnet quote, and checks revocation", { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    await page.getByRole("status").filter({ hasText: /Copied|Prompt selected/ }).waitFor();
    ...
    await page.getByRole("link", { name: "OpenQuick Pro ↗", exact: true }).waitFor({ state: "visible" });
    await page.setViewportSize({ width: 1280, height: 900 });
    await page.getByRole("link", { name: "OpenQuick Pro ↗", exact: true }).click();
    ...
    assert.equal(await page.locator("#save-viewers").isVisible(), false);
    assert.equal(await page.locator("#new-project").isVisible(), false);
    assert.equal(await page.locator("svg[aria-label=MPP] path").first().evaluate(el => getComputedStyle(el).fill), "rgb(23, 23, 23)");
