...
test("native Pro checkout isolates its Host session from public content, stages a mainnet quote, and checks revocation", { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    let checkoutOrigin = "";
    const commonsServer = serve({ port: 0, hostname: "0.0.0.0", fetch: () => new Response(`<!doctype html><form method="POST" action="${checkoutOrigin}/pro/hosting/session"><input name="ticket" value="${ticket}" type="hidden"><button>Open Pro checkout</button></form>`, { headers: { "content-type": "text/html" } }) });
    ...
    for (const path of ["/pro/hosting", "/pro/hosting-client.js", "/api/v1/private-projects", "/sites/anything/", "/agent.md"]) assert.equal((await app.request(`${checkoutOrigin}${path}`)).status, 404);
    ...
    await Promise.all([page.waitForURL(`${checkoutOrigin}/pro/hosting`), page.getByRole("button", { name: "Open Pro checkout" }).click()]);
    ...
    active = false;
    assert.equal((await context.request.get(`${checkoutOrigin}/pro/hosting`)).status(), 404);
